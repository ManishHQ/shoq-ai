...
const TOOLS = [
	// Product Management Tools
	{
		...
		description:
			'Search for products in the catalog using keywords, category, or filters',
		inputSchema: {
			type: 'object',
			properties: {
				query: {
				...
				category: {
					type: 'string',
					description:
						'Filter by category (electronics, clothing, grocery, home, books, etc.)',
					enum: [
						'electronics',
						'clothing',
						'grocery',
		...
		inputSchema: {
			...
			properties: {
				transactionId: {
					type: 'string',
					description:
						'Hedera transaction ID (format: 0.0.XXXXX@TIMESTAMP.NANOSECONDS)',
				},
				chatId: {
					type: 'number',
